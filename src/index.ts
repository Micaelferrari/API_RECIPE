...
app.get('/recipes/:title', async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const recipe = await connection('recipes').where('title', 'ILIKE', `%${title}%`);
...
app.patch('/recipes/:id_recipe/ingredients/:id_ingredient', async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const recipeExists = await connection('recipes').where('id_recipe', id_recipe).first();
    const ingredientExists = await connection('ingredients').where('id_ingredient', id_ingredient).first();
    ...
    // Atualiza a quantidade do ingrediente na receita
...
app.put('/recipes/:id', async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      const recipeExists = await connection('recipes').where('id_recipe', id).first();
...
app.patch('/ingredients/:id', async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  const { name_ingredient, type_ingredient } = req.body;
  ...
  try {
    ...
    await connection('ingredients').where('id_ingredient', id).update({ name_ingredient, type_ingredient });
    res.status(200).json({ message: "Ingredient updated successfully!" });
  } catch (error: any) {
...
// DELETAR INGREDIENTE
app.delete('/ingredients/:id', async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
